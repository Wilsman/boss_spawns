...
const pendingMap = (
  <div className="boss-map-loading" role="status">
    Loading spawn map…
  </div>
);

export default function SpawnResults({
  data,
  mode,
  filters,
  catalog,
  ...
  return (
    <>
      <div hidden={mapView}>
        <ModernTable
          ...
          onOpenMap={(map, boss, location) => {
            ...
            setPopup({ map: map.name, boss, location });
          }}
        />
      </div>
      {mapView &&
        (filtered ? (
          <Suspense fallback={pendingMap}>
            <BossMapViewer
          ...
          <section className="boss-map-picker" aria-label="Choose a spawn map">
            <div>
              <h2>Choose your map</h2>
              <p>Explore possible boss spawns, one map at a time.</p>
            </div>
            <div className="boss-map-map-grid">
              {choices.map((map) => {
                const meta = getMapMeta(map.normalizedName);
                const bossCount = new Set(
                  map.bosses.map((b) => b.mobKey ?? b.boss.name),
                ).size;
                return (
                  <button
                    type="button"
                    ...
                    onClick={() => {
                      const next = new URLSearchParams(params);
                      next.set("map", map.name);
                      setParams(next);
                    }}
                  >
                    <div className="boss-map-preview">
                      {meta?.svgPath ? (
                        <img src={meta.svgPath} alt="" loading="lazy" />
                      ) : (
                        <MapPin size={36} />
                      )}
                    </div>
      ...
      >
        <Dialog.Portal>
          <Dialog.Overlay className="boss-map-dialog-overlay" />
          <Dialog.Content
            className="boss-map-dialog"
            onCloseAutoFocus={(event) => {
              event.preventDefault();
              if (returnFocus.current?.offsetParent)
                returnFocus.current.focus({ preventScroll: true });
            }}
          >
            <Dialog.Title className="sr-only">
              {popup?.boss} spawn locations on {popup?.map}
            </Dialog.Title>
            <Dialog.Description className="sr-only">
              Explore possible spawn positions. Drag to pan and use the zoom
              controls. Escape closes the map.
            </Dialog.Description>
            <Dialog.Close
              className="boss-map-dialog-close"
              aria-label="Close spawn map"
            >
              <X size={19} />
            </Dialog.Close>
            {popupMap && popup && (
              <Suspense fallback={pendingMap}>
                <BossMapViewer
                  key={`${mode}:${popup.map}:${popup.boss}:${popup.location}`}
                  data={popupMap}
                  mode={mode as GameMode}
                  initialBoss={popup.boss}
                  initialLocation={popup.location}
                  onFullMap={(boss) => {
                    const next = new URLSearchParams(params);
                    next.set("view", "map");
                    next.set("map", popup.map);
